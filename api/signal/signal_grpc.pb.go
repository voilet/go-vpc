...
const (
	SignalService_Connect_FullMethodName = "/api.signal.SignalService/Connect"
)
...
func (c *signalServiceClient) Connect(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[SignalMessage, SignalMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SignalService_ServiceDesc.Streams[0], SignalService_Connect_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type SignalService_ConnectClient = grpc.BidiStreamingClient[SignalMessage, SignalMessage]
...
func (UnimplementedSignalServiceServer) Connect(grpc.BidiStreamingServer[SignalMessage, SignalMessage]) error {
...
func _SignalService_Connect_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(SignalServiceServer).Connect(&grpc.GenericServerStream[SignalMessage, SignalMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SignalService_ConnectServer = grpc.BidiStreamingServer[SignalMessage, SignalMessage]
...
var SignalService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "api.signal.SignalService",
	HandlerType: (*SignalServiceServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "Connect",
			Handler:       _SignalService_Connect_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
